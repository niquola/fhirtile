...
(def request
  (-> clj-http.lite.core/request
      clj-http.lite.client/wrap-query-params
      clj-http.lite.client/wrap-user-info
      clj-http.lite.client/wrap-url
      clj-http.lite.client/wrap-redirects
      clj-http.lite.client/wrap-decompression
      clj-http.lite.client/wrap-input-coercion
      clj-http.lite.client/wrap-output-coercion
      clj-http.lite.client/wrap-basic-auth
      clj-http.lite.client/wrap-accept
      clj-http.lite.client/wrap-accept-encoding
      clj-http.lite.client/wrap-content-type
      clj-http.lite.client/wrap-form-params
      clj-http.lite.client/wrap-method
      clj-http.lite.client/wrap-unknown-host))

(defn -main [dir]
  (doseq [f (.listFiles (io/file dir))]
    (let [suit (yaml/parse-string (slurp (.getPath ^java.io.File f)))]
      (println "========================\nExecute " (.getName ^java.io.File f) "\n--------------------------")
      (println (:desc suit))
      (doseq [[nm case] (:cases suit)]
        (let [req (cond-> {:url (str (:base-url suit) (:url case))
                           :headers {"content-type" "application/json"}
                           :method (or (:method case) :get)}
                    (:body case) (assoc :body (json/generate-string (:body case)))
                    (:basic-auth suit) (assoc :basic-auth (:basic-auth suit)))
              res (request req)
              res (update res :body #(json/parse-string % keyword))]

          (when-let [m (:match case)]
            (let [errs (matcho.core/match* res m)]
              (if-not (empty? errs)
                (do 
                  (println "* " (name nm) ": FAIL")
                  (println "Req:" )
                  (println (yaml/generate-string req))
                  (println "Resp:" )
                  (println (yaml/generate-string res))

                  (doseq [er errs]
                    (println "ERROR: "
                             (yaml/generate-string er))))
                (println "* " (name nm) ": Ok!")))))))))
...
;; (defn -main [ & args]
;;   (doseq [tfname args]
;;     (let [_ (println "Processing file: " tfname)
...
;; (comment 
;;   (-main "tests/test1.yaml" "tests/test2.yaml"))
